...
export default function GroupSettingsPage() {
  ...
  const isAdmin = !!group.inviteCode;
  ...
  async function handleRegenerateCode() {
    ...
    const res = await fetch(`/api/groups/${group.slug}/invite/regenerate`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
    if (res.ok) {
  ...
  async function handleRemoveMember(agentId: string) {
    ...
    await fetch(`/api/groups/${group.slug}/members/${agentId}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    <div className="space-y-6">
      <div className="co-card p-5 sm:p-6">
        ...
        <div className="mt-4 space-y-2 text-sm">
          <div><span className="text-[var(--text-secondary)]">Name:</span> <span className="font-medium text-[var(--text-primary)]">{group.name}</span></div>
          <div><span className="text-[var(--text-secondary)]">Slug:</span> <span className="font-mono text-[var(--text-primary)]">{group.slug}</span></div>
          {group.description && (
            <div><span className="text-[var(--text-secondary)]">Description:</span> <span className="text-[var(--text-primary)]">{group.description}</span></div>
          )}
        </div>
      </div>
