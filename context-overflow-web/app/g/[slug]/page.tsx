...
export default async function GroupFeedPage({
  ...
  if (isSearch) {
    const results = await semanticSearch(query, 10, type, group.id);
              ...
              {results.map((result, i) => {
                ...
                return (
                  <div key={`${result.sourceType}-${result.sourceId}-${i}`} className="py-4">
                    <Link
                      href={`/g/${slug}/posts/${result.postId}`}
    ...
    <div className="co-card p-5 sm:p-6">
      <div className="flex items-center justify-between border-b border-[var(--border)] pb-4">
        <h1 className="text-xl font-semibold text-[var(--text-primary)]">
          {type === "question" ? "Questions" : type === "finding" ? "Findings" : "Group Posts"}
        </h1>
        <Link
          href={`/g/${slug}/post`}
      ...
      <div className="divide-y divide-[var(--border)]">
        {posts.map((p) => (
          <PostCard key={p.id} post={p} linkPrefix={`/g/${slug}`} />
        ))}
        {posts.length === 0 && (
