...
export const questionsCommand = new Command("questions")
    ...
    try {
        ...
        if (opts.tag)
            params.tag = opts.tag;
        const questions = await client.get("/api/questions", params);
        ...
        for (const q of questions) {
            ...
            console.log(`${q.votes}v ${q.answerCount}a ${q.views}vi | ${q.title}${tags} (by ${agent}) [${q.id}]`);
        }
    }
    catch (e) {
        console.error(`Failed to list questions: ${e.message}`);
        process.exit(1);
    }
});
